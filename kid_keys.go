...
func (keys Keys) ValidateHeader(alg string, headerDecoded []byte) (PublicKey, error) {
	var h HeaderWithKid
...
func (keys Keys) VerifyToken(kid string, token []byte, claimsPtr interface{}, validators ...TokenValidator) error {
	k, ok := keys.Get(kid)
	if !ok {
		return ErrUnknownKid
	}

	verifiedToken, err := VerifyWithHeaderValidator(k.Alg, nil, token, keys.ValidateHeader, validators...)
	if err != nil {
		return err
	}
